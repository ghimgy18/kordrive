...
if (isset($_POST['submit'])) {
  ...
  $sql = "SELECT * FROM instructor WHERE email='$email'"; // query to check if email already exist or not

  $result = mysqli_query($conn, $sql); // execute the query on the parent database

  if (!$result->num_rows > 0) { // if no same email onthe table 

    $sql = "INSERT INTO instructor (username,phonenumber,email,password)
              VALUES ('$username','$phonenumber','$email','$password')"; //insert value query
...
<body>
  ...
  <div class="row">
    ...
    <form method="post" class="col text-center">
      <h3 class="mb-2" style="text-align: start;">Add Instructor</h3>

      <div class="row mb-4">
        <div class="col">
          <div class="form-outline">
            <input required name="username" type="text" id="form3Example1" class="form-control" />
            <label class="form-label" for="form3Example1">user name</label>
          </div>
        </div>
        <div class="col">
          <div class="form-outline">
            <input required name="password" type="password" id="form3Example2" class="form-control" />
